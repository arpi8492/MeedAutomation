...
Given user launches "Meed" app in "<Platform>" device
 ...
 And user clicks on button "Done"
 And user waits for "5" seconds
 And user selects option "<Payee_name>" from the list
 And user enters text "<New Amount>" in textbox "PAYMENT_AMOUNT"
 And user clicks on button "Confirm_Details"
 And user validates that "Payment_edited_successfully" is displayed
 And user clicks on button "OK" 
 And user selects option "<Payee_name>" from the list
 And user validates "PAYMENT_AMOUNT" field with expected value as "$<New Amount>"
